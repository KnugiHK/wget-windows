...
if [ ! -f "$INSTALL_PATH"/lib/libgnutls.a ]; then
  wget -nc https://www.gnupg.org/ftp/gcrypt/gnutls/v3.8/gnutls-3.8.2.tar.xz
  tar -xf gnutls-3.8.2.tar.xz
  cd gnutls-3.8.2 || exit
...
if [ ! -f "$INSTALL_PATH"/lib/libgpgme.a ]; then
  wget -nc https://gnupg.org/ftp/gcrypt/gpgme/gpgme-1.22.0.tar.bz2
  tar -xf gpgme-1.22.0.tar.bz2
  cd gpgme-1.22.0 || exit
  ./configure \
  --host=$WGET_MINGW_HOST \
  --disable-shared \
  --prefix="$INSTALL_PATH" \
...
if [ ! -f "$INSTALL_PATH"/lib/libz.a ]; then
  wget -nc https://github.com/madler/zlib/releases/download/v1.3/zlib-1.3.tar.gz
